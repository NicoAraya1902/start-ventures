-- Fix security issue: Require authentication for profile discovery
-- Update get_discovery_profiles to only work for authenticated users
...
-- Create additional policy to ensure discovery only works for authenticated users
CREATE POLICY "Discovery profiles only for authenticated users" 
ON public.profiles 
FOR SELECT 
TO authenticated
USING (
  -- Allow viewing basic profile data for discovery purposes
  -- This policy works with the security definer function above
  true
);