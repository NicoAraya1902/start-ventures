-- Add seeking_technical_preference column to profiles table
ALTER TABLE public.profiles 
ADD COLUMN seeking_technical_preference text 
CHECK (seeking_technical_preference IN ('tecnico', 'no_tecnico', 'sin_preferencia'));

-- Drop existing functions first
DROP FUNCTION public.get_discovery_profiles();
DROP FUNCTION public.get_connected_profile_details(uuid);

-- Recreate get_discovery_profiles function with the new field
CREATE OR REPLACE FUNCTION public.get_discovery_profiles()
 RETURNS TABLE(user_id uuid, full_name text, user_type text, entrepreneur_type text, team_status text, project_stage text, project_sector text, project_name text, project_description text, responsible_areas text[], seeking_areas text[], team_members jsonb, support_areas text[], interests text[], hobbies text[], avatar_url text, is_technical boolean, team_size integer, year integer, experience_years integer, seeking_technical_preference text, created_at timestamp with time zone)
 LANGUAGE sql
...
-- Recreate get_connected_profile_details function with the new field
CREATE OR REPLACE FUNCTION public.get_connected_profile_details(target_user_id uuid)
 RETURNS TABLE(user_id uuid, full_name text, email text, phone text, location text, region text, university text, career text, profession text, user_type text, avatar_url text, project_name text, project_description text, responsible_areas text[], seeking_areas text[], team_members jsonb, interests text[], support_areas text[], seeking_technical_preference text)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
